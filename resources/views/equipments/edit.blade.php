...
<header class="max-w-lg mx-auto">
    <a href="#">
        <h1 class="text-4xl font-bold text-white text-center">Edit {{$equipment->name}}</h1>
    </a>
</header>
<main class="bg-white max-w-lg mx-auto p-8 my-10 rounded-lg shadow-2xl">
    <div class=""><a href="{{url('/admin/equipment')}}"> <button type="button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded float-left">Back</button></a>
    </div>
    <section class="mt-10">
        <form action="/admin/equipment/{{$equipment->id}}" method="POST" class="flex flex-col" enctype="multipart/form-data"> 
        @method('PUT')
        @csrf
                <div class="mt-5 mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="name" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$equipment->name}}>
                    @error('name')
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="category" class="block text-gray-700 text-sm font-bold 
                        ...
                        <select class="form-control" name="category">
                            ...
                            <option value="therapy" {{ old('category',$equipment)=='therapy' ? 'selected' : ''  }}>Therapy</option>
                            <option value="massage" {{ old('category',$equipment)=='massage' ? 'selected' : ''  }}>Massage</option>
                        </select>
                    @error('category')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="quantity" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$equipment->quantity}}>
                    @error('quantity')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                    </p>
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="expiration" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$equipment->expiration}}>
                    @error('expiration')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                    </p>
                ...
                <div class="mt-5 mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="description" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$equipment->description}}>
                    @error('description')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                    </p>
