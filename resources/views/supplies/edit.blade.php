...
<header class="max-w-lg mx-auto">
    <a href="#">
        <h1 class="text-4xl font-bold text-white text-center">Edit {{$supply->name}}</h1>
    </a>
</header>
<main class="bg-white max-w-lg mx-auto p-8 my-10 rounded-lg shadow-2xl">
    ...
    <section class="mt-10">
        <form action="/admin/supplies/{{$supply->id}}" method="POST" class="flex flex-col" enctype="multipart/form-data"> 
        @method('PUT')
        @csrf
                <div class="mt-5 mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="name" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$supply->name}}>
                    @error('name')
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="category" class="block text-gray-700 text-sm font-bold 
                        ...
                        <select class="form-control" name="category">
                            ...
                            <option value="therapy" {{ old('category',$supply)=='therapy' ? 'selected' : ''  }}>Therapy</option>
                            <option value="massage" {{ old('category',$supply)=='massage' ? 'selected' : ''  }}>Massage</option>
                        </select>
                    @error('category')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="quantity" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$supply->quantity}}>
                    @error('quantity')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                    </p>
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="expiration" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$supply->expiration}}>
                    @error('expiration')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                    </p>
                ...
                <div class="mt-5 mb-6 pt-3 rounded bg-gray-200"> 
                    <label for="description" class="block text-gray-700 text-sm font-bold 
                        ...
                        focus:outline-none border-b-4 border-gray-400 px-3" autocomplete="off" value={{$supply->description}}>
                    @error('description')
                    <p class="text-red-500 text-xs p-1">
                        {{$message}}
                    </p>
